...
/*   Updated: 2026/07/30 18:57:32 by aelbouaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	extract_frame(t_sprite *sprite, int f, int a)
{
	...
	while (y < sprite->frame_h)
	{
		...
		{
			pixel = &sprite->sheet->pixels
			[((sprite->anims[a].row * sprite->frame_h + y)
					* sprite->sheet->width + (f * sprite->frame_w + x)) * 4];
...
static void	load_all_frames(mlx_t *mlx, t_sprite *sprite, int max_frames)
{
	...
	while (a < ANIM_COUNT)
	{
		...
		{
			sprite->frames[a * max_frames + f] = mlx_new_image(mlx,
